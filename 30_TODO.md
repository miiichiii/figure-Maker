...
- [done][P1-High] 手のひらツール選択中にカーソルが手のひら表示にならない不具合を修正（`hand` 時は常に `grab`、ドラッグ中は `grabbing`）
...
- [done][P1-High] 細線オブジェクトを選択して移動しただけで `stroke-width` が太る回帰を修正（drag finishで非平行移動transformのBakeを抑制）
